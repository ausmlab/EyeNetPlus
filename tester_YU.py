                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_test):
                            ...
                            pc_path = dataset.path + '/original_ply/' + name[:-4] + '.ply'
                            print(pc_path)
                            xyz, i, _ = DP.read_ply_YU(pc_path)
                            write_ply(join(test_path, 'test_preds', name), [xyz, i, preds, labels], ['x','y','z', 'intensity', 'n_return', 'pred', 'label'])
