...
class Network:
        ...
        for i in range(self.config.num_layers):
            if i == 0:
                ...
                b_f_encoder_i = self.dilated_res_block(b_f_encoder_i, inputs['b_xyz'][i], inputs['b_neigh_idx'][i],
                                                          64,
                                                          'Encoder_layer_' + str(i) + '_b', is_training, '0')
                m_f_encoder_i = self.dilated_res_block(m_f_encoder_i, inputs['m_xyz'][i], inputs['m_neigh_idx'][i],
                                                          64,
                                                          'Encoder_layer_' + str(i) + '_m', is_training, '0')
            else:
                ...
                for j in range((d_out[i]-d_out[i-1])//32) :
                    b_f_encoder_i_cb = self.dilated_res_block(b_f_encoder_i, inputs['b_xyz'][i], inputs['b_neigh_idx'][i], 32,
                                                     'Encoder_layer_' + str(i) + '_b', is_training, str(j))
                    m_f_encoder_i_cb = self.dilated_res_block(m_f_encoder_i, inputs['m_xyz'][i], inputs['m_neigh_idx'][i], 32,
                                                     'Encoder_layer_' + str(i)+ '_m', is_training, str(j))
                    b_f_encoder_i = tf.concat([b_f_encoder_i, b_f_encoder_i_cb], axis=-1)
                    m_f_encoder_i = tf.concat([m_f_encoder_i, m_f_encoder_i_cb], axis=-1)
            
            #print ("b_f_encoder_i", i, b_f_encoder_i.get_shape) ## YJ
    ...
    def get_loss(self, logits, labels, pre_cal_weights):
        ...
        if self.loss_type == 'lovas':
            print (100*'#')
            print ('lovas loss will be applied')
            print (100*'#')
    ...
    def building_no_drb_block(self, xyz, feature, neigh_idx, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value
        f_xyz = self.relative_pos_encoding(xyz, neigh_idx)
        f_xyz = helper_tf_util.conv2d(f_xyz, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
