...
tag = 'PoickingSample_v2'

class ConfigSensatUrban:
    exp_name = 'Sensat_' + tag
    log_file_name = 'lovas'
    ...
    num_points = 7168 # 28672 # Number of input points 
...
class ConfigDALES:
    ...
    k_n = [16, 21, 21, 21, 16]  # KNN
    num_layers = 5  # Number of layers
    num_points = 7168 #50176#114688#28672#57344 # Number of input points
    num_classes = 8  # Number of valid classes
    sub_grid_size = 0.2 #0.20 # preprocess_parameter

    batch_size = 32 #8#8 # batch_size during training
    val_batch_size = 128 #32#32  # batch_size during validation and test
    train_steps = 1200 # 500 # Number of steps per epochs
    val_steps = 50  # Number of validation steps per epoch
    ...
    n_point_connection = [1024, 256, 64, 16, 4]  #[4096, 1024, 256, 64, 32]#[7168, 1792, 448, 112, 28]#[16384, 4096, 1024, 256, 128]#[8192, 2048, 512, 128, 64] #Number of Connection Points for each layers

    noise_init = 3.5  # noise initial parameter
    # max_epoch should be decided considering batch_size since 1epoch consists of 500 steps regardless of batch_sie
    max_epoch = 100  # maximum epoch during training
    learning_rate = 1e-2  # initial learning rate
    lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
...
class ConfigToronto3D:
    ...
    d_out = [64, 128, 256, 448, 512]
    #d_out = [16, 64, 128, 256, 512]  # feature dimension
    n_point_connection = [4096, 1024, 256, 64, 16] # [1024, 256, 64, 16, 4] #[4096, 1024, 256, 64, 16] #Number of Connection Points for each layers

    noise_init = 3.5  # noise initial parameter
    max_epoch = 100  # maximum epoch during training
    learning_rate = 5e-3 #1e-2  # initial learning rate
    lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
