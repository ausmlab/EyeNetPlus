...
from tester_Semantic3D import ModelTester, ModelTester_validation
...
class Semantic3D:
    def __init__(self):
        ...
        self.sub_pc_folder = join(self.path, 'grid_{:.3f}'.format(cfg.sub_grid_size))
        ...
        for pc_name in cloud_names:
            if exists(join(self.original_folder, pc_name + '.labels')):
                self.train_file_name.append(join(self.sub_pc_folder, pc_name + '.ply'))
            else:
                #print(join(self.full_pc_folder, pc_name + '.ply'))
                if 'reduced' in pc_name:
                    self.test_file_name.append(join(self.full_pc_folder, pc_name + '.ply'))
                
        self.train_file_name = np.sort(self.train_file_name)
        self.test_file_name = np.sort(self.test_file_name)
        
        for i, file_path in enumerate(self.train_file_name):
            if self.all_splits[i] == self.val_split:
                self.val_file_name.append(file_path)
        
        self.train_file_name = np.sort([x for x in self.train_file_name if x not in self.val_file_name])
    ...
    def load_sub_sampled_clouds(self, sub_grid_size):
        ...
        self.all_files = np.hstack((self.train_file_name, self.val_file_name, self.test_file_name))
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            elif file_path in self.train_file_name:
            ...
            if cloud_split == 'test':
                #print("processing test projection")
                proj_file = join(tree_path, '{:s}_proj.pkl'.format(cloud_name))
                with open(proj_file, 'rb') as f:
                    proj_idx, labels = pickle.load(f)
